...
import {deletePost, setSimilarPosts} from "../../store/reducers/AnnouncementSlice";
...
const Details:React.FunctionComponent = () => {
    ...
    const postSplitDesc = post.description.toLowerCase().replace(',', ' ').split(' ')

    function findSimilar() {
        const similarList = announcements.filter(value => {
        ...
        <div>
            <Card className={"container_item"}>
                ...
                <Container className={"edit_buttons"}>
                    ...
                    <button className="btn btn-danger px-3 btn_dt "
                            onClick={()=>{
                                deleteFunc(post.id)
                                navigate('/')
                            }}
                    ...
                    <Link to={`/similarPosts`}>
                        <button className="btn btn-secondary px-3 btn_dt "
                                onClick={()=>{
                                    findSimilar()
                                }}
                        > Show top 3 similar </button>
                    </Link>
                    <button className="btn btn-secondary px-3 btn_dt "
                    onClick={()=>navigate("/")}
